...
async def counter_test(dut):
    ...
    for i in range(5):
        ...
        dut._log.info(f"{i:4d}\t{dut.rst_n.value}\t{dut.enable.value}\t{dut.count.value:3d}")
...
async def counter_enable_test(dut):
    ...
    assert int(dut.count.value) == 1, f"Counter should be 1 after one cycle, got {dut.count.value}"
